...
class _PhysicsCalibrationScreenState extends State<PhysicsCalibrationScreen> {
  ...
  void _handleSave() async {
    ...
    final shot = PingPongShot(
      topMotorSpeed: _topMotor,
      bottomMotorSpeed: _bottomMotor,
      horizontalAngle: 0,
      interval: 1.0,
    );

    if (mqtt.isConnected) {
      final success = await mqtt.sendShotCommand(shot);
      if (!success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Error al enviar configuración'),
    ...
    await db.insertSession(
      shot: shot,
      playerName: 'Calibración',
      preset: _spinType.toLowerCase(),
    );
  ...
  Widget _buildHeader() {
    return Column(
      ...
      children: [
        ...
        Text(
          'Ajuste fino de la rotación de la pelota',
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  '${_spinDiff >= 0 ? '+' : ''}${_spinDiff.toStringAsFixed(0)} RPM',
